...
CREATE POLICY "media_select_own" ON storage.objects FOR SELECT TO authenticated
  USING (bucket_id IN ('produtos','personagens') AND (storage.foldername(name))[1] = auth.uid()::text);
CREATE POLICY "media_insert_own" ON storage.objects FOR INSERT TO authenticated
  WITH CHECK (bucket_id IN ('produtos','personagens') AND (storage.foldername(name))[1] = auth.uid()::text);
